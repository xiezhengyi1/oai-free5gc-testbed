...
def _maximum_observed(values: list[float | None]) -> float | None:
    observed = [value for value in values if value is not None]
    return max(observed) if observed else None


def _prb_utilization(
    values: list[float | None], capacity: float
) -> float | None:
    observed = [value for value in values if value is not None]
    if len(observed) != len(values):
        return None
    return sum(observed) / capacity
...
class SnapshotBuilder:
    ...
    def _ran_metrics(
        ...
        selector = _label_selector(
            {"gnb": gnb_id, "ue_id": ue_id, "supi": supi}
        )
        result: dict[str, Any] = {}
    ...
    def _build(self, trigger_event: str) -> UnifiedSnapshot:
        ...
        gnb_resource_views = {
            gnb.id: resource_view(
                ...
                prb_utilization=_prb_utilization(
                    [
                        ran_by_ue[ue.id].get("prb_ul")
                        for ue in self.scenario.ues
                        if ue.serving_gnb == gnb.id
                    ],
                    float(self.scenario.ran.bandwidth_rb),
                ),
            )
