...
test "$(git -C "$oai_root" describe --tags --exact-match)" = v2.4.0
...
docker build --target oai-gnb --tag oaisoftwarealliance/oai-gnb:v2.4.0 \
  --file "$oai_root/docker/Dockerfile.gNB.ubuntu" "$oai_root"
docker build --target oai-nr-ue --tag oaisoftwarealliance/oai-nr-ue:v2.4.0 \
